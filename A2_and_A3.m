...
A1 = R1.*exp(1i*deg2rad(0));

% Основной цикл
for i=1:count

    A2 = R2.*exp(1i*deg2rad(range_a2(i)));
...
surf(range_a2, range_a3, abs(result_AKF), 'MeshStyle','column','LineWidth',0.1,'EdgeAlpha',0.2);
...
surf(range_a2, range_a3, abs(result_VKF), 'MeshStyle','column','LineWidth',0.1,'EdgeAlpha',0.2);
zlabel('Значение бокового пика','FontSize',14);
ylabel('Угол фазы коэффциента a_1','FontSize',14);
xlabel('Угол фазы коэффциента a_2','FontSize',14);
